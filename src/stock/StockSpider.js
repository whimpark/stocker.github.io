...
function toJsonFiles(data){
    data.forEach(e => {
        ...
        for (let index = 0; index < e.klines.length; index++) {
            ...
            if(isChange) {
                FileHelper.write(file, kline)  
            } else{
                FileHelper.append(file, "\n"+kline)
            }
            lastValue=klineYear
        }
        let path=PATH+"/stock/spider/company/"+e.code+".json"
        e.klines=[]
        FileHelper.write(path, JSON.stringify(e, null, 2))
    });
}
...
async function main(){
    ...
    if(argv["local"]=="true"){
        ...
        // let code="601136"
        // let klinesFiles=FileHelper.getFiles("./data/stock/spider/klines"+"/"+code)
